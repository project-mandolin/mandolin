...
import org.mitre.mandolin.mlp.{TanHLType, ReluLType, LType}
...
/**
  ...
  * A MMLP model setting can be passed in so that all the static (unchanging) learning
...
class ModelSpace(_realMPs: Vector[RealMetaParameter], _catMPs: Vector[CategoricalMetaParameter],
  ...
  def getSpec(lsp: Tuple4Value[CategoricalValue, IntValue, RealValue, RealValue]): LType = {
    val lt = lsp.v1.s match {
      case "TanHLType" => TanHLType
      case _ => ReluLType
    }
    val dim = lsp.v2.v
    val l1 = lsp.v3.v
