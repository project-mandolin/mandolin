...
class ANNetwork(val inLayer: InputLayer, val layers: IndexedSeq[NonInputLayer], sparseInput: Boolean) extends Serializable {
    ...
    if (!allZero) {
      ...
      var i = 0; while (i < curDim) {
        var j = 0; while (j < prevDim) {
          if (util.Random.nextBoolean()) w.update(i, j, getGaussian(rv, gv).toFloat)
          j += 1
        }
        i += 1
      }
   ...
   * @author wellner
   */
  def generateRandomWeights: MMLPWeights = {
    ...
    new MMLPWeights(new MMLPLayout(wts))
  }
   ...
   * @author wellner
   */
  def generateZeroedLayout: MMLPLayout = {
   ...
   */
  def forwardPass(inputVec: Vec, targetVec: Vec, glpW: MMLPWeights, training: Boolean = true): Unit = {
   ...
   *  @return MMLPLayout object representing the gradient as a sequence of (matrix, vector) pairs
   */
  def getGradient(inputVec: Vec, targetVec: Vec, glpW: MMLPWeights) = {
    forwardPass(inputVec, targetVec, glpW, true)
    backpropGradients(inputVec, targetVec, glpW)
  }
  
  def backpropGradients(inputVec: Vec, targetVec: Vec, glpW: MMLPWeights) = {
    if ((numLayers > 1) || !sparseInput) {
      val revGrads = for (i <- (numLayers - 1) to 0 by -1) yield {
        val (w, b) = glpW.wts.get(i)
        layers(i).getGradient(w, b) // backward pass and return gradient at that layer
      }
      new MMLPLayout(revGrads.reverse) // return in order from input to penultimate layer
    } else {
      val (w, b) = glpW.wts.get(0)
      new MMLPLayout(IndexedSeq(outLayer.getGradientWith(inputVec, targetVec, w, b)))
    }
  }
