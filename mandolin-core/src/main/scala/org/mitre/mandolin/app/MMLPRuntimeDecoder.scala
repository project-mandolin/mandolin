...
class MMLPRuntimeDecoder(filePath: String, io: IOAssistant, posCase: String) extends RuntimeDecoder {
  ...
  val predictor = new CategoricalMMLPPredictor(model.ann, true)
  val evalDecoder = new org.mitre.mandolin.predict.standalone.PosteriorDecoder(fe, predictor, new MMLPPosteriorOutputConstructor)
