...
class TestSelection extends FlatSpec with Matchers {
  ...
  "A random evaluator simulation" should "evaluate concurrently" in {
    ...
    val nt : Broadcast[Vector[MMLPFactor]] = sc.broadcast(Vector())
    val ntst : Broadcast[Vector[MMLPFactor]] = sc.broadcast(Vector())
    ...
    val scoring = new MockScoringFunction
    val scorerActor = system.actorOf(Props(new ModelScorer(modelSpace, scoring, master, 100, 20, 100,1)), name = "scorer")
