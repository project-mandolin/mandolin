...
object XGMain extends org.mitre.mandolin.config.LogInit with org.mitre.mandolin.app.AppMain {
  ...
  def getVecIOs(appSettings: XGModelSettings): (Vector[MMLPFactor], Option[Vector[MMLPFactor]], Int, Int) = {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val (finalMetric, booster) = evaluator.evaluateTrainingSet(trVecs.toIterator, tstVecs map {
      _.toIterator
    })
    booster foreach { b =>
